...
public class Propulsion extends TImedMotor implements MoveListener {
	...
	public Propulsion() {
		...
		left = WheeledChassis
				.modelWheel(new EV3LargeRegulatedMotor(LocalEV3.get().getPort(R2D2Constants.LEFT_WHEEL)), DIAMETER)
				.offset(-1 * R2D2Constants.DISTANCE_TO_CENTER);
		right = WheeledChassis
				.modelWheel(new EV3LargeRegulatedMotor(LocalEV3.get().getPort(R2D2Constants.RIGHT_WHEEL)), DIAMETER)
				.offset(R2D2Constants.DISTANCE_TO_CENTER);
	...
	public void stopMoving() {
		pilot.setLinearAcceleration(R2D2Constants.LINEAR_ACCELERATION_STOP);
		pilot.stop();
		pilot.setLinearAcceleration(R2D2Constants.LINEAR_ACCELERATION);
	}
	 ...
	 *            vrai si à gauche
	 * @param correction
	 *            vrai si on doit appliquer une correction de 2%
	 */
	...
	public void orientateSouth(boolean left) {
		double orientate = left ? getRotateToNorth() - R2D2Constants.SOUTH : getRotateToNorth() + R2D2Constants.SOUTH;
		if (orientate < 0)
			pilot.rotate(orientate, false);
		else
			pilot.rotate(orientate, true);
	...
	public void moveStopped(Move event, MoveProvider mp) {
		...
		orientation = orientation
				+ (event.getAngleTurned() + (event.getAngleTurned() * R2D2Constants.PR_ANGLE_CORRECTION));
	...
	public boolean hasRunXPercentOfLastRun(int percentage) {
		return System.currentTimeMillis() > startTime + ((lastRunTime * (percentage / 2)) / 100);
	}
